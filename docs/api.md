...
class Matcher:
    ...
    H_tilde: set[tuple[Vertex, Vertex]]
    S_hat: set[Vertex]
    k: int
    accountant: Ledger
```
...
class Vizing:
    """Deterministic fan-based Vizing/Misra--Gries colourer."""
...
def augment(
    matching: set[Edge], neighbors: callable, start: Vertex, is_matched: callable
) -> bool: ...
```
...
def random_updates(
    n: int, steps: int, rng: random.Random, existing: set[tuple[int, int]] | None = None
) -> Iterator[Update]: ...
...
def run_parallel(
    configs: list[tuple[int, str, int, int]], max_workers: int | None = None
) -> list[Benchmark]: ...
def compare(
    n: int, updates: int, seed: int = 42, max_workers: int | None = None
) -> dict[str, Benchmark]: ...
```
